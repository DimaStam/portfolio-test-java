...
public class CheckPromotionsFilterTest extends TestCase {
    ...
    @Test(enabled = false, description = "Check Promotions checkbox")
    public void checkPromotionCheckbox() {
        ...
        int promotedProductsFilterCount = listingPage.getPromotedProductsFilterCount("Promocja");

        assertThat(listingPage.getFiltersArea()).isVisible();

        listingPage
                .selectPromotedProductsCheckbox("Promocja")
                .clickAcceptFiltersButton()
                .waitForProductsGrid();

        Assert.assertEquals(promotedProductsFilterCount, listingPage.getProductsPageCount());
    }
}