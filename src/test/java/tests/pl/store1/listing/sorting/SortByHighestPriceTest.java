...
public class SortByHighestPriceTest extends TestCase {
    ...
    @Test(description = "Check sorting by highest price")
    public void checkSortByHighesPrice() {
        ...
        listingPage
                .openSortingDropdown()
                .clickSortByHighestPriceAndWaitForResponsePL("store1_pl_simple_products_price_default_desc")
                .waitForProductsGrid();

        boolean arePricesValid = listingPage.checkIsPricesSortedDesc(listingPage.getProductPrices());
        if (!arePricesValid) {
            fail("Ceny nie są posortowane od największej do najmniejszej");
        }
    }
}