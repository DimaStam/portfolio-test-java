...
public class CheckStoreAvailabilityTest extends TestCase {
    ...
    @Test(description = "Check product availability in stores")
    public void checkAvailabilityInStores() {
        ...
        assertThat(productPage.getAvailabilityInStoresButton()).isVisible(new LocatorAssertions.IsVisibleOptions().setTimeout(15000));

        productPage.clickStoreAvailabilityButton();

        assertThat(productPage.getAvailabilityTab()).isVisible(new LocatorAssertions.IsVisibleOptions().setTimeout(10000));

        productPage.searchStoreByAdress(ProductCardData.ADRESS_NAME.getValue());

        assertThat(productPage.getOnlyInStoreCheckbox()).isVisible();
        assertThat(productPage.getSearchStoreName()).hasText(ProductCardData.STORE_NAME.getValue());

        productPage.closeStoreAvailabilityTab();

        assertThat(productPage.getAvailabilityTab()).isHidden();
    }
}