...
public class DisplayLinkAndNavigateToPaymentMethodPageTest extends TestCase {
    ...
    @Test(description = "Display link and navigate to payment method page")
    public void displayLinkAndNavigateToPaymentMethodPage(){
        Footer footer = new Footer(page);
        assertThat(footer.getFooterContentSection()).isVisible();
        assertThat(footer.getLinkAriaLabelAttribute(LinksTexts.PAYMENT_LABEL_PL.getValue())).isVisible();

        (new FooterMenu(page)).navigateToPaymentMethodPage();

        assertThat(page)
                .hasTitle(PageTitles.PAYMENT_TITLE_PL
