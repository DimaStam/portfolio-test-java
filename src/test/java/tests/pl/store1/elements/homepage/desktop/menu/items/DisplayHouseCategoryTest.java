...
public class DisplayHouseCategoryTest extends TestCase {
    ...
    @Test(description = "Display house category link in main menu")
    public void displayHouseCategory(){

        CategoryMenu categoryMenu = (new CategoryMenu(page))
                .clickMenuRightArrow();

        assertThat(categoryMenu.menu(MenuLinksTexts.HOUSE_URL_PL.getValue())).isVisible();

        categoryMenu
                .expandSubMenu(MenuLinksTexts.HOUSE_URL_PL.getValue())
                .navigateToCategoryPage();

        Assert.assertTrue(page.url().contains(MenuLinksTexts.HOUSE_URL_PL.getValue()));
