...
public class DisplayLinkAndNavigateToPrivacyPolicyPageTest extends TestCase {
    ...
    @Test(description = "Display link and navigate to privacy policy page")
    public void testDisplayLinkAndNavigateToPrivacyPolicyPage(){
        Footer footer = new Footer(page);
        assertThat(footer.getFooterContentSection()).isVisible();
        assertThat(footer.getLinkAriaLabelAttribute(LinksTexts.PRIVACY_POLICY_LABEL_PL.getValue())).isVisible();

        (new FooterMenu(page)).navigateToPrivacyPolicyPage();

        assertThat(page)
                .hasTitle(PageTitles.PRIVACY_POLICY_TITLE_PL
