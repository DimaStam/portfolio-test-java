...
public class DisplayLinkAndNavigateToShopRegulationPageTest extends TestCase {
    ...
    @Test(description = "Display link and navigate to shop regulation page")
    public void testDisplayLinkAndNavigateToShopRegulationPage() {
        Footer footer = new Footer(page);
        assertThat(footer.getFooterContentSection()).isVisible();
        assertThat(footer.getLinkAriaLabelAttribute(LinksTexts.REGULATION_LABEL_PL.getValue())).isVisible();

        (new FooterMenu(page)).navigateToShopRegulationPage();

        assertThat(page)
                .hasTitle(PageTitles.REGULATION_TITLE_PL
