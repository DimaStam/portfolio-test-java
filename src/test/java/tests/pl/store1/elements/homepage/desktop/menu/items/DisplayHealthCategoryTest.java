...
public class DisplayHealthCategoryTest extends TestCase {
    ...
    @Test(description = "Display health category link in main menu")
    public void displayHealthCategory(){

        CategoryMenu categoryMenu = (new CategoryMenu(page))
                .clickMenuRightArrow();

        assertThat(categoryMenu.menu(MenuLinksTexts.HEALTH_URL_PL.getValue())).isVisible();

        categoryMenu
                .expandSubMenu(MenuLinksTexts.HEALTH_URL_PL.getValue())
                .navigateToCategoryPage();

        Assert.assertTrue(page.url().contains(MenuLinksTexts.HEALTH_URL_PL.getValue()));
