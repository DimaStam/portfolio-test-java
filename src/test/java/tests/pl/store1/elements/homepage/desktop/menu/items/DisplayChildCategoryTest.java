...
public class DisplayChildCategoryTest extends TestCase {
    ...
    @Test(description = "Display child category link in main menu")
    public void displayChildCategory(){

        CategoryMenu categoryMenu = (new CategoryMenu(page));

        assertThat(categoryMenu.menu(MenuLinksTexts.CHILD_URL_PL.getValue())).isVisible();

        categoryMenu
                .expandSubMenu(MenuLinksTexts.CHILD_URL_PL.getValue())
                .navigateToCategoryPage();

        Assert.assertTrue(page.url().contains(MenuLinksTexts.CHILD_URL_PL.getValue()));
