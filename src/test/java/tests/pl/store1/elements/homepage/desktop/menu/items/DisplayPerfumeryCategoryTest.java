...
public class DisplayPerfumeryCategoryTest extends TestCase {
    ...
    @Test(description = "Display perfumery link in main menu")
    public void displayPerfumeryCategory(){

        CategoryMenu categoryMenu = (new CategoryMenu(page));

        assertThat(categoryMenu.menu(MenuLinksTexts.PERFUMERY_URL_PL.getValue())).isVisible();

        categoryMenu
                .expandSubMenu(MenuLinksTexts.PERFUMERY_URL_PL.getValue())
                .navigateToCategoryPage();

        Assert.assertTrue(page.url().contains(MenuLinksTexts.PERFUMERY_URL_PL.getValue()));
